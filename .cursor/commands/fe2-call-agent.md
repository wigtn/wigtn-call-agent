...
**v2 변경사항**:
- FE1의 채팅 화면에서 바로 `/calling/[id]`로 이동
- 기존 `/confirm/[id]` 확인 화면 삭제됨 (채팅에서 확인 처리)
...
- **받는 것**:
  - FE1에서 `/calling/[id]`로 이동 (**채팅 화면에서 직접 이동**)
  - BE1이 만든 API (`/api/calls/[id]`)
  - BE2가 업데이트하는 통화 결과 (polling으로 감지)
- **주는 것**: 결과 화면에서 홈으로 이동 (채팅 화면 = `/`)
- **NOTE**: v2에서는 `/confirm/[id]` 경로가 없음 (채팅에서 확인 처리)
